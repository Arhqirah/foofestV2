  ...
  return (
    <>
      <Section title={null} customStyle={`!m-0 !p-0 w-full max-w-[1600px] place-self-center place-items-center`}>
        <figure className="flex z-0 relative h-[50dvh] w-full max-h-[1/2]">
          <div className="w-full h-full aspect-square xsm:aspect-video">
              ...
              ? (<Image src={`${data.logo}${bigSizes[1]}x${bigSizes[0]}?random=${random}`} fill={true} alt={data.name} priority={true}></Image>)
      ...
      <Section title={null} customStyle={`m-0 p-0 w-full max-w-[1600px] place-self-center place-items-center`}>
        <article className="flex flex-col gap-6">
          <div className="grid md:grid-cols-2 gap-6">
            <figure className="w-full object-fill">
              {data.logo.startsWith("http") ?  
                (<Image src={`${data.logo}${smallSizes[0]}x${smallSizes[1]}?metal=${random}`} height={800} width={800} alt={data.name} priority={false}></Image>)
                :
        ...
        {/* <article className="grid grid-rows-2 gap-6 w-full ">
          <div className="grid sm:grid-cols-2 md:mx-auto sm:w-3/4">
